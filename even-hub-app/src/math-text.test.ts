...
describe('LaTeX math to G2-readable text', () => {
  ...
  it('handles display, inline, bracket, and parenthesis delimiters', () => {
    const input = String.raw`Inline $x_i$ and \(y^2\). Display: $$\sum_{i=1}^{n} i^2$$. Also \[\int_a^b f(x)\,dx\].`

    assert.equal(input.includes('$x_i$'), true)
    assert.equal(
      normalizeMathForDisplay(input),
      'Inline x_(i) and y². Display: ∑[i = 1..n] i². Also ∫[a..b] f(x) dx.',
    )
  })

  it('keeps adjacent math expressions readable as separate expressions', () => {
    assert.equal(normalizeMathForDisplay(String.raw`\(x_i\)\(y_i\)`), 'x_(i) y_(i)')
    assert.equal(normalizeMathForDisplay(String.raw`$a$$b$`), 'a b')
  })
